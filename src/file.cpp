...
boolean openFile(char *path) {
  ...
  if (fr == FR_OK) {
    fr = f_stat(path, &fno);  // ファイルサイズ取得 ※外すと f_seek が正しく動かないので注意
    filesize = fno.fsize;
  ...
  switch (command) {
    ...
    case 0x66:
      if (!vgmLoopOffset) {  // ループしない曲
        vgmEnd = true;
        if (numFiles[currentDir] - 1 == currentFile) openDirectory(1); else vgmPlay(1);
      } else {
        vgmLoop++;
...
//----------------------------------------------------------------------
// count 個あとのディレクトリを開いて最初のファイルを再生。マイナスは前のディレクトリ
