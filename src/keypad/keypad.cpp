...
KeypadClass::KeypadClass() {
  ...
  RCU_CFG0 |=
      (0b10 << 14) | (1 << 28);  // ADC clock = 108MHz / 8 = 13.5MHz(14MHz max.)
...
byte KeypadClass::readButton() {
  ...
  */
  ...
  if (adcData < 1600 && adcData >= 1200)
    return btnDOWN;  // 1375 - 1385 , 戻り値 2
  if (adcData < 2250 && adcData >= 2000)
    return btnLEFT;  // 2051 - 2065 , 戻り値 3
  if (adcData < 3200 && adcData >= 2900)
    return btnSELECT;  // 2985 - 2995 , 戻り値 4
  return btnNONE;
}
...
byte KeypadClass::checkButton() {
  ...
  if (ms >
      buttonLastTick + BUTTON_INTERVAL) {  // 最後に押したときから
